(function () {
  ...
  class Input {
    ...
    setKey(key, active) { this.keys[key] = active; }
    tapKey(key) {
      this.prev[key] = false;
      this.keys[key] = true;
      window.setTimeout(() => { this.keys[key] = false; }, 120);
    }
  }
  ...
  function menu() {
    if (window.top && window.top !== window) window.top.postMessage({ type: "codex-menu-exit" }, window.location.origin);
      ...
      state = freshState();
      state.mode = selected;
      state.message = "Back at the siege menu. Press Enter to launch.";
    }
  }
  ...
  function setupStandaloneUi() {
    ...
    if (fullscreenButton) {
      fullscreenButton.addEventListener("click", () => {
        const target = document.querySelector(".game-frame") || document.documentElement;
    ...
    for (const button of document.querySelectorAll("[data-touch-key]")) {
      ...
      button.addEventListener("pointerdown", (event) => {
        event.preventDefault();
        audioReady();
        if (key === " " || key === "q") input.setKey(key, true);
        else input.tapKey(key);
      });
      button.addEventListener("pointerup", () => input.setKey(key, false));
      button.addEventListener("pointercancel", () => input.setKey(key, false));
      button.addEventListener("pointerleave", () => input.setKey(key, false));
    }

    const movePad = document.getElementById("movePad");
    if (movePad) {
      const clearMove = () => {
        for (const key of ["w", "a", "s", "d"]) input.setKey(key, false);
      };
      const setMove = (event) => {
        ...
        const dead = 14;
        clearMove();
        input.setKey("a", dx < -dead);
        input.setKey("d", dx > dead);
        input.setKey("w", dy < -dead);
        input.setKey("s", dy > dead);
        input.mouse.x = clamp(state.player.x + dx * 4, 0, W);
        input.mouse.y = clamp(state.player.y + dy * 4, 0, H);
      };
