...
export class InfernoBehemothBossSystem {
  constructor(scene, world, ui, feedback = null) {
    this.scene = scene;
    this.world = world;
    this.ui = ui;
    this.feedback = feedback;
    this.config = SETTINGS.bosses.infernoBehemoth;
  ...
  createBoss() {
    const boss = {
      ...
      patrolIndex: 0,
      chargeCooldown: 3.4,
      lavaBurstCooldown: 4.6,
      attackCooldown: 0,
      stateTimer: 0,
      hitReactTimer: 0,
      chargeDirection: new THREE.Vector3(),
      home: new THREE.Vector3(-146, 0, 150),
      patrolPoints: [
        new THREE.Vector3(-146, 0, 150),
        new THREE.Vector3(-138, 0, 145),
        new THREE.Vector3(-148, 0, 137),
        new THREE.Vector3(-156, 0, 147),
      ],
    ...
    boss.group.traverse((child) => {
      if (child.isMesh) {
        child.userData.infernoBehemothBoss = boss;
  ...
  createBossMesh() {
    ...
    const hide = new THREE.MeshStandardMaterial({ color: 0x4b3f3d, roughness: 0.94 });
    const dark = new THREE.MeshStandardMaterial({ color: 0x17191d, roughness: 0.82, metalness: 0.08 });
    const hornMat = new THREE.MeshStandardMaterial({ color: 0xff9a42, roughness: 0.58, emissive: 0x8d2208, emissiveIntensity: 0.22 });
    const glow = new THREE.MeshStandardMaterial({ color: 0xffc35b, roughness: 0.34, emissive: 0xff4a12, emissiveIntensity: 0.72 });
    const dust = new THREE.MeshStandardMaterial({ color: 0x8d3d2d, roughness: 0.9 });
    ...
    [-1, 1].forEach((side) => {
      ...
      shoulderWeak.name = "infernoBehemoth-weakspot";
    ...
    backWeak.name = "infernoBehemoth-weakspot";
  ...
  update(deltaSeconds, player) {
    ...
    boss.lavaBurstCooldown = Math.max(0, boss.lavaBurstCooldown - deltaSeconds);
    ...
    if (boss.state === "charge") {
    ...
    } else if (boss.state === "lavaBurst") {
      this.updateLavaBurst(deltaSeconds, boss);
      ...
      } else if (distance <= this.config.lavaBurstDistance && boss.lavaBurstCooldown <= 0) {
        this.startLavaBurst(boss);
      } else {
  ...
  startCharge(boss, target) {
    ...
    this.feedback?.spawnImpact(boss.group.position.clone().add(new THREE.Vector3(0, 0.8, 0)), 0x8d3d2d, 1.85);
  ...
  updateCharge(deltaSeconds, boss) {
    ...
    if (boss.stateTimer <= 0) {
      ...
      this.feedback?.spawnImpact(boss.group.position.clone().add(new THREE.Vector3(0, 0.7, 0)), 0xff6a1d, 2.6);
      this.feedback?.shake(0.2);
    }
  }

  startLavaBurst(boss) {
    boss.state = "lavaBurst";
    boss.stateTimer = 1.05;
    boss.lavaBurstCooldown = this.config.lavaBurstCooldown;
    boss.hitReactTimer = 0.3;
    this.feedback?.spawnImpact(boss.group.position.clone().add(new THREE.Vector3(0, 0.9, 0)), 0xff6a1d, 3.05);
    this.feedback?.playSound("enemyHit", 0.78);
    this.feedback?.shake(0.18);
  }

  updateLavaBurst(deltaSeconds, boss) {
    boss.stateTimer -= deltaSeconds;
    if (boss.stateTimer <= 0) {
      boss.state = "recover";
      boss.stateTimer = this.config.recoveryDuration;
  ...
  animate(boss) {
    ...
    if (boss.group.userData.glow) {
      boss.group.userData.glow.emissiveIntensity = boss.state === "lavaBurst" || boss.hitReactTimer > 0 ? 1.1 : 0.72;
    }
  }

  handleArrowHit(arrow) {
    const boss = arrow.hitObjectRef?.userData.infernoBehemothBoss;
    ...
    const typeBonus = arrow.arrowType?.id === "ice" ? 1.12 : arrow.arrowType?.id === "fire" ? 0.9 : 1;
    ...
    this.feedback?.spawnImpact(arrow.hitPoint ?? boss.group.position, boss.health <= 0 ? 0xff6a1d : (weakSpot ? 0xff6a1d : 0x8d3d2d), boss.health <= 0 ? 4 : (weakSpot ? 2.25 : 1.45));
    this.feedback?.shake(boss.health <= 0 ? 0.36 : 0.075 + shotPower * 0.08);
    this.showBossCombatText(boss, boss.health <= 0 ? "INFERNO BEHEMOTH FALLS" : (weakSpot ? `WEAK ${Math.round(damage)}` : `${Math.round(damage)}`), boss.health <= 0 ? "xp" : "damage strong");
  ...
  handleAreaArrowEffect(arrow) {
    ...
    this.showBossCombatText(boss, boss.health <= 0 ? "INFERNO BEHEMOTH FALLS" : `${Math.round(damage)}`, boss.health <= 0 ? "xp" : "damage strong");
    if (boss.health <= 0) this.defeatBoss(boss);
  }
  ...
  defeatBoss(boss) {
    ...
    this.feedback?.spawnImpact(boss.group.position.clone().add(new THREE.Vector3(0, 1.2, 0)), 0xff6a1d, 4.35);
    ...
    window.dispatchEvent(new CustomEvent("echo-archer:rare-loot", {
      detail: { name: "Behemoth Embercore", rarity: "epic", text: "Rare volcanic gear and proof of the Ember Peak victory." },
    }));
    this.defeatListeners.forEach((callback) => callback({ boss, type: "infernoBehemoth" }));
  }
