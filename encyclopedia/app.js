...
const FACTS_PER_STANDARD_CATEGORY = 50;
const GENERATED_ASPECTS = ["history","features","uses","study","impact"];
const CATEGORY_TOPIC_ROOTS = {
  "countries": [
    "Canada",
    "Brazil",
    "India",
...
const CATEGORY_PURPOSES = {
  "technology": "human-made tools, systems, and methods for solving problems",
  "transportation": "ways people and goods move between places",
  "weather": "short-term conditions in the atmosphere",
  "geology": "Earth materials, structures, and processes",
  "minerals": "naturally occurring solid substances with characteristic composition",
  "dinosaurs": "extinct reptiles that lived during the Mesozoic Era",
  "human-body": "organs, tissues, and systems in people",
  "medicine": "health care, disease prevention, diagnosis, and treatment",
  "chemistry": "matter, substances, and reactions",
  "physics": "matter, energy, forces, and motion",
  "economics": "production, exchange, and choices about scarce resources",
  "law": "rules, courts, rights, and legal systems",
  "education": "teaching, learning, and schools",
  "agriculture": "growing crops and raising animals for food and materials",
  "architecture": "the design and construction of buildings and structures",
  "religions": "belief systems, practices, texts, and communities",
  "mythology": "traditional stories about gods, heroes, and origins",
  "explorers": "people known for travel, mapping, and discovery",
  "famous-people": "notable people and their contributions",
...
function categoryDistractors(category) {
  return CATEGORY_ORDER.filter((id) => id !== category).slice(0, 3).map((id) => CATEGORY_LABEL_OVERRIDES[id] || titleCase(id.replace(/-/g, " ")));
}
...
Object.entries(CATEGORY_TOPIC_ROOTS).forEach(([category, roots]) => {
  const existingTopics = new Set(RAW_FACTS.filter((item) => item.category === category).map((item) => item.topic));
  const needed = Math.max(0, FACTS_PER_STANDARD_CATEGORY - countFacts(category));
  generatedTopicsFor(category, roots)
    .filter((topic) => !existingTopics.has(topic))
    .slice(0, needed)
    .forEach((topic) => RAW_FACTS.push(generatedTopicFact(category, topic)));
});

const PAGE_SIZE = 72;
const CATEGORY_DEFS = [
  { id: "all", label: "All Categories", count: RAW_FACTS.length },
  ...CATEGORY_ORDER.map((id) => ({ id, label: CATEGORY_LABEL_OVERRIDES[id], count: countFacts(id) }))
];
...
function entry(category, topic, text, source, question, answer, distractors, detail) {
  const sourceObject = typeof source === "string" ? { label: source, url: "" } : source;
  return {
    category,
    topic,
    text,
    detail: detail || `Learn more: ${text} This fact is grouped here so you can study ${topic} alongside related entries without leaving the encyclopedia.`,
...
function simpleFacts(category, source, rows) {
  return rows.map(([topic, text, question, answer, distractors, detail]) => entry(category, topic, text, source, question, answer, distractors, detail));
}
...
function categoryLabel(categoryId) {
  return CATEGORY_DEFS.find((category) => category.id === categoryId)?.label || titleCase(categoryId.replace(/-/g, " "));
}
...
function renderFacts() {
  ...
  els.sectionMeta.textContent = state.search
    ...
    : state.sort === "default"
      ? "World Capitals keeps all 245 alphabetized capitals. Every other section now has 50 facts plus in-app Show more details."
...
function renderFactCard(item) {
  const source = item.url
    ? `<a href="${escapeHtml(item.url)}" rel="noreferrer" target="_blank">${escapeHtml(item.proof)}</a>`
...
function newQuestion() {
  ...
  els.answerGrid.innerHTML = answers.slice(0, 4).map((answer) => `<button type="button" data-answer="${escapeHtml(answer)}">${escapeHtml(answer)}</button>`).join("");
}
